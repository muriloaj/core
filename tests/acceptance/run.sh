...
if [ -z "${APPS_TO_DISABLE}" ] && [ "${RUNNING_WEBUI_TESTS}" = true ]
then
	# By default, disable these apps when running webUI tests
...
# Re-enable apps that were disabled
for APP_TO_ENABLE in ${APPS_TO_REENABLE}; do
	remote_occ ${ADMIN_AUTH} ${OCC_URL} "--no-warnings app:enable ${APP_TO_ENABLE}"
done

# Re-disable apps that were enabled
